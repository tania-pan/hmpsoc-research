...
architecture rtl of asp_peak is
    ...
    type state_type is (STATE_INIT, STATE_WAIT_DATA, STATE_PROCESS_CHECK, STATE_SEND_COUNT);
...
begin
    ...
    begin
        if reset = '0' then
            ...
            if peak_enabled = '1' then
                if global_cycle_counter /= x"FFFFFF" then
            ...
            if current_state = STATE_SEND_COUNT then
                global_cycle_counter <= (others => '0');
            end if;
        end if;
    end process;
        ...
        if reset = '0' then
            ...
            case current_state is
                ...
                when STATE_WAIT_DATA =>
                    if noc_recv(39) = '1' and noc_recv(23) = '1' and peak_enabled = '1' then
                        samp_n2 <= samp_n1;
                        samp_n1 <= samp_n0;
                ...
                when STATE_PROCESS_CHECK =>
                    if (samp_n1 > samp_n2) and (samp_n1 > samp_n0) then
                        final_captured_count <= std_logic_vector(global_cycle_counter);
                        current_state        <= STATE_SEND_COUNT;
                    else
                        current_state        <= STATE_WAIT_DATA;
                    end if;

                when STATE_SEND_COUNT =>
                    if noc_ready = '1' then
                        noc_send(39)           <= '1';           
