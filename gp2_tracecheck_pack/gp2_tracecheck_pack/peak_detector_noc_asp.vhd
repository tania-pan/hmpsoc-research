...
-- Output: MSG_DATA passthrough for debug/visualisation on every input sample.
--         MSG_EVENT when a local peak is detected. Payload contains the counter value.
...
begin
    ...
    process(clk, reset_n)
        ...
        variable event_payload_v: std_logic_vector(31 downto 0);
    begin
        if reset_n = '0' then
            enabled         <= '1';
            ...
            if noc_recv(39) = '1' and noc_recv(38 downto 35) = MSG_DATA and enabled = '1' and noc_ready = '1' then
                current_corr_v := unsigned(noc_recv(31 downto 0));
                event_payload_v := std_logic_vector(counter);

                if current_corr_v > last_corr then
                    positive_slope <= '1';
                    counter <= counter + 1;
                    -- Pass through correlation so final_pkt visibly changes in simulation/board debug.
                    noc_send <= make_packet(MSG_DATA, next_dest, noc_recv(31 downto 0));

                elsif current_corr_v < last_corr and positive_slope = '1' then
                    -- Falling after a rising slope = local peak.
                    noc_send       <= make_packet(MSG_EVENT, next_dest, event_payload_v);
                    peak_reg       <= '1';
                    counter        <= (others => '0');
                    positive_slope <= '0';
