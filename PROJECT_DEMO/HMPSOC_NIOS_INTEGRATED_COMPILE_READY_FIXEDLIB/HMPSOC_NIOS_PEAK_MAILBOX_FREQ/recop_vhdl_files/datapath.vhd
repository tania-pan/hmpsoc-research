...
entity datapath is
    port (
        ...
        -- Exposed data-memory bus for memory-mapped peripherals.
        -- Normal data RAM still lives inside this datapath.
        -- External logic can decode addresses such as 0x3000-0x3007.
        dm_ext_addr   : out bit_16;
        dm_ext_wdata  : out bit_16;
        dm_ext_rdata  : in  bit_16;
        dm_ext_wren   : out bit_1;
...
architecture rtl of datapath is
    ...
    -- DATA MEMORY SIGNALS
    signal dm_addr_full_s : std_logic_vector(15 downto 0);
    ...
    signal dm_ram_wren_s  : std_logic;
    signal dm_ext_sel_s   : std_logic;
...
begin
    ...
    -- Full 16-bit address is exposed so the wrapper can decode
    -- memory-mapped peripherals above the internal 4K data RAM.
    with dm_addr_sel select
        dm_addr_full_s <= std_logic_vector(rz_s)        when "00",
                          std_logic_vector(rx_s)        when "01",
                          std_logic_vector(ir_operand)  when "10",
                          (others => '0')              when others;

    dm_addr_s    <= dm_addr_full_s(11 downto 0);
    dm_ext_sel_s <= '1' when dm_addr_full_s(15 downto 12) = x"3" else '0';

    -- Internal RAM is disabled for memory-mapped peripheral writes.
    dm_ram_wren_s <= dm_wr when dm_ext_sel_s = '0' else '0';

    -- External bus exported to recop_top / NoC wrapper.
    dm_ext_addr  <= bit_16(dm_addr_full_s);
    dm_ext_wdata <= bit_16(dm_data_s);
    dm_ext_wren  <= dm_wr when dm_ext_sel_s = '1' else '0';

    -- Reads from 0x3xxx come from the external peripheral bus; all other
    -- addresses come from internal data RAM.
    dm_out_s <= dm_ext_rdata when dm_ext_sel_s = '1' else bit_16(dm_q_s);
